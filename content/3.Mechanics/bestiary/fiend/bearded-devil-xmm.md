...
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/XMM/Bearded%20Devil.webp#right)
...
***Beard.*** *Melee Attack Roll:* `dice:1d20+5|noform|noparens|text(+5)`, reach 5 ft. *Hit:* `dice:1d8+3|noform|noparens|avg|text(7)` (`1d8 + 3`) Piercing damage, and the target has the [Poisoned](3.Mechanics/rules/conditions.md#Poisoned) condition until the start of the devil's next turn. Until this poison ends, the target can't regain [Hit Points](3.Mechanics/rules/variant-rules/hit-points-xphb.md).

***Infernal Glaive.*** *Melee Attack Roll:* `dice:1d20+5|noform|noparens|text(+5)`, reach 10 ft. *Hit:* `dice:1d10+3|noform|noparens|avg|text(8)` (`1d10 + 3`) Slashing damage. If the target is a creature and doesn't already have an infernal wound, it is subjected to the following effect. *Constitution Saving Throw:* DC 12. *Failure:* The target receives an infernal wound. While wounded, the target loses `dice:1d10|noform|noparens|avg|text(5)` (`1d10`) [Hit Points](3.Mechanics/rules/variant-rules/hit-points-xphb.md) at the start of each of its turns. The wound closes after 1 minute, after a spell restores [Hit Points](3.Mechanics/rules/variant-rules/hit-points-xphb.md) to the target, or after the target or a creature within 5 feet of it takes an action to stanch the wound, doing so by succeeding on a DC 12 Wisdom ([Medicine](3.Mechanics/rules/skills.md#Medicine)) check.
```
^statblock
