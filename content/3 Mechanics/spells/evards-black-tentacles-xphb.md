...
Each creature in that area makes a Strength saving throw. On a failed save, it takes `dice:3d6|noform|noparens|avg` (`3d6`) Bludgeoning damage, and it has the [Restrained](conditions.md#Restrained) condition until the spell ends. A creature also makes that save if it enters the area or ends its turn there. A creature makes that save only once per turn.
