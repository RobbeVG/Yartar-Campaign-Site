...
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/XMM/Mummy%20Lord.webp#right)
...
***Magic Resistance.*** The mummy has [Advantage](3.Mechanics/rules/variant-rules/advantage-xphb.md) on saving throws against spells and other magical effects.

***Undead Restoration.*** If destroyed, the mummy gains a new body in 24 hours if its heart is intact, reviving with all its [Hit Points](3.Mechanics/rules/variant-rules/hit-points-xphb.md). The new body appears in an unoccupied space within the mummy's lair. The heart is a Tiny object that has AC 17, HP 10, and [Immunity](3.Mechanics/rules/variant-rules/immunity-xphb.md) to all damage except Fire.

## Actions

***Multiattack.*** The mummy makes one Rotting Fist or Channel Negative Energy attack, and it uses Dreadful Glare.

***Rotting Fist.*** *Melee Attack Roll:* `dice:1d20+9|noform|noparens|text(+9)`, reach 5 ft. *Hit:* `dice:2d10+4|noform|noparens|avg|text(15)` (`2d10 + 4`) Bludgeoning damage plus `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) Necrotic damage. If the target is a creature, it is cursed. While cursed, the target can't regain [Hit Points](3.Mechanics/rules/variant-rules/hit-points-xphb.md), it gains no benefit from finishing a [Long Rest](3.Mechanics/rules/variant-rules/long-rest-xphb.md), and its [Hit Point](3.Mechanics/rules/variant-rules/hit-points-xphb.md) maximum decreases by `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) every 24 hours that elapse. A creature dies and turns to dust if reduced to 0 [Hit Points](3.Mechanics/rules/variant-rules/hit-points-xphb.md) by this attack.
...
***Dreadful Glare.*** *Wisdom Saving Throw:* DC 17, one creature the mummy can see within 60 feet. *Failure:* `dice:6d6+4|noform|noparens|avg|text(25)` (`6d6 + 4`) Psychic damage, and the target has the [Paralyzed](3.Mechanics/rules/conditions.md#Paralyzed) condition until the end of the mummy's next turn.
...
**At will:** [Dispel Magic](3.Mechanics/spells/dispel-magic-xphb.md), [Thaumaturgy](3.Mechanics/spells/thaumaturgy-xphb.md)

**1/day each:** [Animate Dead](3.Mechanics/spells/animate-dead-xphb.md), [Harm](3.Mechanics/spells/harm-xphb.md), [Insect Plague](3.Mechanics/spells/insect-plague-xphb.md) (level 7 version)

## Reactions

***Whirlwind of Sand.*** Trigger: The mummy is hit by an attack roll. _Response:_ The mummy adds 2 to its AC against the attack, possibly causing the attack to miss, and the mummy teleports up to 60 feet to an unoccupied space it can see. Each creature of its choice that it can see within 5 feet of its destination space has the [Blinded](3.Mechanics/rules/conditions.md#Blinded) condition until the end of the mummy's next turn.
...
***Dread Command.*** The mummy casts [Command](3.Mechanics/spells/command-xphb.md) (level 2 version), using the same spellcasting ability as Spellcasting. The mummy can't take this action again until the start of its next turn.
