...
***Ram.*** *Melee Attack Roll:* `dice:1d20+5|noform|noparens|text(+5)`, reach 5 ft. *Hit:* `dice:1d6+3|noform|noparens|avg|text(6)` (`1d6 + 3`) Bludgeoning damage. If the target is a Large or smaller creature and the elk moved 20+ feet straight toward it immediately before the hit, the target takes an extra `dice:1d6|noform|noparens|avg|text(3)` (`1d6`) Bludgeoning damage and has the [Prone](3.Mechanics/rules/conditions.md#Prone) condition.
```
^statblock
