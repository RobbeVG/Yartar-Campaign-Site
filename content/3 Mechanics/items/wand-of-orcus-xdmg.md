...
Any creature besides Orcus that tries to attune to the wand makes a DC 17 Constitution saving throw. On a successful save, the creature takes `dice:10d6|noform|noparens|avg` (`10d6`) Necrotic damage. On a failed save, the creature dies and, if it is a Humanoid, turns into a [Zombie](zombie-xmm.md).

## Magic Weapon

You can wield the wand as a magic [Mace](mace-xphb.md) that grants a +3 bonus to attack rolls and damage rolls made with it. The wand deals an extra `dice:2d12|noform|noparens|avg` (`2d12`) Necrotic damage on a hit.
