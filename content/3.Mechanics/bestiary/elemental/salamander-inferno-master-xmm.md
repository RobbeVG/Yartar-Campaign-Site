...
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/XMM/Salamander%20Inferno%20Master.webp#right)
...
***Fire Aura.*** At the end of each of the salamander's turns, each creature of the salamander's choice in a 10-foot [Emanation](3.Mechanics/rules/variant-rules/emanation-area-of-effect-xphb.md) originating from the salamander takes `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) Fire damage.

***Magic Resistance.*** The salamander has [Advantage](3.Mechanics/rules/variant-rules/advantage-xphb.md) on saving throws against spells and other magical effects.

## Actions

***Multiattack.*** The salamander makes two Flame Trident attacks.

***Flame Trident.*** *Melee  or Ranged Attack Roll:* `dice:1d20+12|noform|noparens|text(+12)`, reach 5 ft. or range 30/90 ft. *Hit:* `dice:2d8+7|noform|noparens|avg|text(16)` (`2d8 + 7`) Piercing damage plus `dice:4d6|noform|noparens|avg|text(14)` (`4d6`) Fire damage. *Hit or Miss:* The trident magically returns to the salamander's hand immediately after a ranged attack.

***Inferno Blast (Recharge 5-6).*** *Dexterity Saving Throw:* DC 18, each creature in a 30-foot-radius [Sphere](3.Mechanics/rules/variant-rules/sphere-area-of-effect-xphb.md) centered on a point the salamander can see within 120 feet. *Failure:* `dice:10d6|noform|noparens|avg|text(35)` (`10d6`) Fire damage, and the target starts [burning](3.Mechanics/traps-hazards/burning-xphb.md), taking `dice:1d10|noform|noparens|avg|text(5)` (`1d10`) Fire damage at the start of each of its turns instead of the normal [burning](3.Mechanics/traps-hazards/burning-xphb.md) damage. The target gains 1 [Exhaustion](3.Mechanics/rules/conditions.md#Exhaustion) level whenever it takes this [burning](3.Mechanics/traps-hazards/burning-xphb.md) damage. *Success:* Half damage only.

## Bonus Actions

***Blazing Movement.*** The salamander moves up to its [Speed](3.Mechanics/rules/variant-rules/speed-xphb.md) without provoking [Opportunity Attacks](3.Mechanics/rules/actions.md#Opportunity%20Attack). During this movement, fire fills a 5-foot [Emanation](3.Mechanics/rules/variant-rules/emanation-area-of-effect-xphb.md) originating from the salamander. When the [Emanation](3.Mechanics/rules/variant-rules/emanation-area-of-effect-xphb.md) enters a creature's space, that creature takes `dice:2d6|noform|noparens|avg|text(7)` (`2d6`) Fire damage. A creature can take this damage only once per turn.
```
^statblock
