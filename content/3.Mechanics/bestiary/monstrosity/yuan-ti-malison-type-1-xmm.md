...
***Multiattack.*** The yuan-ti makes two attacks, using Bite or Poison Ray in any combination, and it can use Spellcasting to cast [Suggestion](3.Mechanics/spells/suggestion-xphb.md) if available.

***Bite.*** *Melee Attack Roll:* `dice:1d20+5|noform|noparens|text(+5)`, reach 5 ft. *Hit:* `dice:1d4+3|noform|noparens|avg|text(5)` (`1d4 + 3`) Piercing damage plus `dice:2d6|noform|noparens|avg|text(7)` (`2d6`) Poison damage.

***Poison Ray (Yuan-ti Form Only).*** *Ranged Attack Roll:* `dice:1d20+5|noform|noparens|text(+5)`, range 120 ft. *Hit:* `dice:2d8+3|noform|noparens|avg|text(12)` (`2d8 + 3`) Poison damage.
