...
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/XMM/Chimera.webp#right)
...
***Bite.*** *Melee Attack Roll:* `dice:1d20+7|noform|noparens|text(+7)`, reach 5 ft. *Hit:* `dice:2d6+4|noform|noparens|avg|text(11)` (`2d6 + 4`) Piercing damage, or `dice:4d6+4|noform|noparens|avg|text(18)` (`4d6 + 4`) Piercing damage if the chimera had [Advantage](3.Mechanics/rules/variant-rules/advantage-xphb.md) on the attack roll.

***Claw.*** *Melee Attack Roll:* `dice:1d20+7|noform|noparens|text(+7)`, reach 5 ft. *Hit:* `dice:1d6+4|noform|noparens|avg|text(7)` (`1d6 + 4`) Slashing damage.

***Ram.*** *Melee Attack Roll:* `dice:1d20+7|noform|noparens|text(+7)`, reach 5 ft. *Hit:* `dice:1d12+4|noform|noparens|avg|text(10)` (`1d12 + 4`) Bludgeoning damage. If the target is a Medium or smaller creature, it has the [Prone](3.Mechanics/rules/conditions.md#Prone) condition.

***Fire Breath (Recharge 5-6).*** *Dexterity Saving Throw:* DC 15, each creature in a 15-foot [Cone](3.Mechanics/rules/variant-rules/cone-area-of-effect-xphb.md). *Failure:* `dice:7d8|noform|noparens|avg|text(31)` (`7d8`) Fire damage. *Success:* Half damage.
```
^statblock
