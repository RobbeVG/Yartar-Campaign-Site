...
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/XMM/Darkmantle.webp#right)
...
***Crush.*** *Melee Attack Roll:* `dice:1d20+5|noform|noparens|text(+5)`, reach 5 ft. *Hit:* `dice:1d6+3|noform|noparens|avg|text(6)` (`1d6 + 3`) Bludgeoning damage, and the darkmantle attaches to the target. If the target is a Medium or smaller creature and the darkmantle had [Advantage](3.Mechanics/rules/variant-rules/advantage-xphb.md) on the attack roll, it covers the target, which has the [Blinded](3.Mechanics/rules/conditions.md#Blinded) condition and is suffocating while the darkmantle is attached in this way.

While attached to a target, the darkmantle can attack only the target but has [Advantage](3.Mechanics/rules/variant-rules/advantage-xphb.md) on its attack rolls. Its [Speed](3.Mechanics/rules/variant-rules/speed-xphb.md) becomes 0, it can't benefit from any bonus to its [Speed](3.Mechanics/rules/variant-rules/speed-xphb.md), and it moves with the target.

A creature can take an action to try to detach the darkmantle from itself, doing so with a successful DC 13 Strength ([Athletics](3.Mechanics/rules/skills.md#Athletics)) check. On its turn, the darkmantle can detach itself by using 5 feet of movement.

***Darkness Aura (1/Day).*** Magical [Darkness](3.Mechanics/rules/variant-rules/darkness-xphb.md) fills a 15-foot [Emanation](3.Mechanics/rules/variant-rules/emanation-area-of-effect-xphb.md) originating from the darkmantle. This effect lasts while the darkmantle maintains [Concentration](3.Mechanics/rules/conditions.md#Concentration) on it, up to 10 minutes. Darkvision can't penetrate this area, and no light can illuminate it.
```
^statblock
