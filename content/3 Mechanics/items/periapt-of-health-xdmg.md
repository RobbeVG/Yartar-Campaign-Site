---
obsidianUIMode: preview
cssclasses:
- json5e-item
tags:
- ttrpg-cli/compendium/src/5e/xdmg
- ttrpg-cli/item/attunement/required
- ttrpg-cli/item/rarity/uncommon
draft: true
title: "Periapt of Health"
aliases:
- "Periapt of Health"
---
# Periapt of Health
*Wondrous item, uncommon (requires attunement)*  
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/items/XDMG/Periapt%20of%20Health.webp#right)

- **Weight**: 1.0 lbs.

While wearing this pendant, you can take a [Magic](actions.md#Magic) action to regain `dice:2d4+2|noform|noparens|avg` (`2d4 + 2`) [Hit Points](hit-points-xphb.md). Once used, this property can't be used again until the next dawn.

In addition, you have [Advantage](advantage-xphb.md) on saving throws to avoid or end the [Poisoned](conditions.md#Poisoned) condition while you wear this pendant.

*Source: Dungeon Master's Guide (2024) p. 284. Available in the <span title='Systems Reference Document (5.2)'>SRD</span> and the Free Rules (2024)*